...
import com.team4.pojo.*;
import com.team4.service.CatelogService;
...
import com.team4.service.WbCommentsService;
import com.team4.service.WbPostService;
...
public class GoodsController {
    ...
    @Resource
    private WbCommentsService wbcommentsService;
    @Resource
    private WbPostService WbPostService;
    ...
    public ModelAndView homeGoods(HttpServletRequest request) throws Exception {
        ...
        //每个种类显示商品数量
        int goodsSize = 6;
        request.getSession().setAttribute("ifhave", false);
    ...
    public ModelAndView getGoodsById(HttpServletRequest request,@PathVariable("id") Integer id,@RequestParam(value = "str",required = false) String str) throws Exception {
    	...
    	//获取商品的详细信息的时候还要获取的是商品的评价消息 comments  根据的商品的id来查找的
    	List<Comments> comments = commentsService.getComByGoodId(id);
    ...
    public ModelAndView gotoForum(HttpServletRequest request) {
    	ModelAndView modelAndView = new ModelAndView();
    	List<WbPost> postLists = WbPostService.getAllPosts();
    	List<WbComments> commentsList = wbcommentsService.getAllComments();
    	modelAndView.addObject("postLists", postLists);
    	modelAndView.addObject("commentsList", commentsList);
    	modelAndView.setViewName("/forum/home");
    	return modelAndView;
    }
    /**
     * 用户删除商品
    ...
    public String publishGoods(HttpServletRequest request) {
        ...
        if(cur_user == null) {
        	...
        	System.out.println("这里是pubGoods。。。。。。");
